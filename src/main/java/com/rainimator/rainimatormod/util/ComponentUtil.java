...
public class ComponentUtil {
    public static List<TextComponent> splitText(TextComponent arg_chatText, int chatWidth, Font fontRenderer) {
        int i = 0;
        TextComponent ichatcomponent = new TextComponent("");
        List<TextComponent> list = Lists.newArrayList();
        List<TextComponent> chatComponents = Lists.newArrayList(arg_chatText);

        for (int j = 0; j < chatComponents.size(); ++j) {
            TextComponent currentChatComponent = chatComponents.get(j);
            String unformattedText = currentChatComponent.getContents();
            ...
            if (unformattedText.contains("\n")) {
                ...
                TextComponent chatcomponenttext = new TextComponent(getColorCode(unformattedText) + s1);
            ...
            TextComponent newChatComponent = new TextComponent(textRemovedLastNewline);
            ...
            if (i + textWidth > chatWidth) {
                ...
                if (s3 != null) {
                    ...
                    TextComponent chatcomponenttext2 = new TextComponent(getColorCode(s2) + s3);
                ...
                newChatComponent = new TextComponent(s2);
                newChatComponent.setStyle(currentChatComponent.getStyle());
                addToList = true;
            }
            if (i + textWidth <= chatWidth) {
            ...
            if (addToList) {
                ...
                i = 0;
                ichatcomponent = new TextComponent("");
            }
        }
        list.add(ichatcomponent);
        return list;
