        ...
        if (Math.random() < 0.2D) {
            ...
            for (int index0 = 0; index0 < (int) num; index0++) {
                if (entity.level instanceof ServerLevel _level)
                    _level.sendParticles((ParticleOptions) ParticleTypes.END_ROD, entity.getX() + r * Math.cos(angel), entity.getY() + 1.0D, entity.getZ() + r * Math.sin(angel), 1, 0.0D, 0.0D, 0.0D, 0.0D);
                angel += 360.0D / num;
            }
        }
        return retval;
