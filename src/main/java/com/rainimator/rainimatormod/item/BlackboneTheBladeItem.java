...
public class BlackboneTheBladeItem extends SwordItem {
    ...
    public boolean hurtEnemy(@NotNull ItemStack itemstack, @NotNull LivingEntity entity, @NotNull LivingEntity sourceentity) {
        ...
        if (Math.random() < 0.1D) {
            ((Entity) entity).hurt(DamageSource.MAGIC, Mth.nextInt(new Random(), 1, 3));
        }
        if (Math.random() < 0.3D) {
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(@NotNull Level world, @NotNull Player entity, @NotNull InteractionHand hand) {
        ...
        if (entity.isShiftKeyDown()) {
            if (!world.isClientSide())
                world.playSound(null, new BlockPos(x, y, z), Objects.requireNonNull(ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation(RainimatorMod.MOD_ID, "blackbone_living"))), SoundSource.NEUTRAL, 10.0F, 1.0F);
            else
                world.playLocalSound(x, y, z, Objects.requireNonNull(ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation(RainimatorMod.MOD_ID, "blackbone_living"))), SoundSource.NEUTRAL, 10.0F, 1.0F, false);
        ...
        if (Math.random() < 0.2D) {
            ...
            for (int index0 = 0; index0 < (int) num; index0++) {
                if (entity.level instanceof ServerLevel _level)
                    _level.sendParticles((ParticleOptions) ParticleTypes.LAVA, entity.getX() + r * Math.cos(angel), entity.getY() + 1.0D, entity.getZ() + r * Math.sin(angel), 1, 0.0D, 0.0D, 0.0D, 0.0D);
                angel += 360.0D / num;
            }
        }
        return retval;
