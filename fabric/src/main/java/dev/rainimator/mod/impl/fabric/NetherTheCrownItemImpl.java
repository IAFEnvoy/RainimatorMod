...
public class NetherTheCrownItemImpl extends NetherTheCrownItem implements ICurio {
    ...
    public void curioTick(String identifier, int index, LivingEntity entity) {
        ICurio.super.curioTick(identifier, index, entity);
        if (entity == null) return;
        Vec3d _center = new Vec3d(entity.getX(), entity.getY(), entity.getZ());
        List<Entity> _entfound = entity.world.getEntitiesByClass(Entity.class, (new Box(_center, _center)).expand(16.0D), e -> true).stream().sorted(Comparator.comparingDouble(_entcnd -> _entcnd.squaredDistanceTo(_center))).toList();
