...
public class PatrickArmorItem extends ArmorWithTickItem {
    ...
    public void onArmorTick(World world, PlayerEntity entity) {
        if (!entity.getWorld().isClient && ServerConfig.getInstance().enableArmorEffect &&
                entity.getEquippedStack(EquipmentSlot.HEAD).isOf(RainimatorItems.PATRICK_HELMET.get()) &&
                entity.getEquippedStack(EquipmentSlot.CHEST).isOf(RainimatorItems.PATRICK_CHESTPLATE.get()) &&
                entity.getEquippedStack(EquipmentSlot.LEGS).isOf(RainimatorItems.PATRICK_LEGGINGS.get()) &&
                entity.getEquippedStack(EquipmentSlot.FEET).isOf(RainimatorItems.PATRICK_BOOTS.get())) {
