...
import com.iafenvoy.rainimator.renderer.model.ModelKingNormalCrown;
...
public class KingNormalCrownRenderer implements IArmorRendererBase<LivingEntity> {
    @Override
    public BipedEntityModel<LivingEntity> getHumanoidArmorModel(LivingEntity livingEntity, ItemStack itemStack, EquipmentSlot equipmentSlot, BipedEntityModel<LivingEntity> bipedEntityModel) {
        BipedEntityModel<LivingEntity> armorModel = new BipedEntityModel<>(new ModelPart(Collections.emptyList(), Map.of(
                "head", new ModelKingNormalCrown<>(MinecraftClient.getInstance().getEntityModelLoader().getModelPart(ModelKingNormalCrown.LAYER_LOCATION)).Head,
